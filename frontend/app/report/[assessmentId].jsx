    ...
    return (<AppScreen>
      ...
      <FadeIn delay={120}>
        <AppCard>
          ...
          {item.assessment.riskFlags.length > 0 ? (item.assessment.riskFlags.map((flag, idx) => (<Text style={styles.body} key={`${flag.code}-${idx}`}>
              - {flag.code} [{flag.severity}] {flag.ingredients.length ? `(${flag.ingredients.join(', ')})` : ''}
            </Text>))) : (<Text style={styles.body}>- No flags recorded.</Text>)}
        </AppCard>
      </FadeIn>
      ...
      <FadeIn delay={160}>
        <AppCard>
          ...
          {item.assessment?.xai?.precautions?.length > 0 ? (item.assessment.xai.precautions.map((precaution, idx) => (<Text style={styles.body} key={`${precaution.code}-${idx}`}>
                - {precaution.code}: {precaution.text}
              </Text>))) : (<Text style={styles.body}>- No precautions returned.</Text>)}
          <Text style={styles.sectionTitle}>Alternatives</Text>
          {item.assessment?.xai?.alternatives?.constraints ? (<Text style={styles.body}>
          ...
          {item.assessment?.xai?.alternatives?.results?.length > 0 ? (item.assessment.xai.alternatives.results.map((alt) => (<Text style={styles.body} key={alt.qr_id}>
                - {alt.product_name} ({alt.qr_id}) score {alt.suitability_score}/100
              </Text>))) : (<Text style={styles.body}>- No alternatives found.</Text>)}
        </AppCard>
      </FadeIn>
